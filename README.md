...
- **UT 검사 절차 시뮬레이터** (`ut_procedure_simulator.html`): V1 블록 입사점 → DAC·기준감도 → transfer correction → 가상 스캐닝·6 dB drop 길이 측정 → 합격 레벨(Fig 13.2.3/13.2.4) 판독까지 5단계를 이어서 수행하는 인터랙티브 시뮬레이션(영문 UI, USM 계열 탐상기 외형, 프로브 각도 45/60/70° 선택) — half-skip 이후의 후속 도달 에코(V1 반복·후벽 미러)도 A-scan에 표시되고, 각 단계의 교정·측정 품질이 최종 판정에 전파되며 디브리핑으로 숨겨진 정답과 비교 (`?selftest`·`?demo=N`·`?seed=`·`?angle=` 지원)
