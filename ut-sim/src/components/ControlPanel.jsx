...
function Knob({ value, min, max, step = 1, onDelta, color = '#39424c', size = 38 }) {
  ...
  return (
    <div
      ...
      style={{
        width: size,
        height: size,
        background: 'radial-gradient(circle at 50% 40%, ' + color + ' 0%, ' + color + ' 58%, #10151a 100%)',
        border: '1px solid #10151a',
        boxShadow: 'inset 0 1px 0 rgb(255 255 255 / 0.18), 0 1px 2px rgb(0 0 0 / 0.5)',
        cursor: 'ns-resize',
      }}
      onPointerDown={(e) => {
...
function Usk7Panel({ state, dispatch, tofdMode, onRecordDac, onAddMarker }) {
  ...
  const lbl = 'text-[9px] font-medium uppercase tracking-[0.06em] text-knob-green'
  const val = 'font-mono text-[10px] text-knob-green'
  return (
    <div className="flex w-[252px] flex-col gap-2 border-l border-[#2a323c] bg-panel-olive p-2 shadow-[inset_0_1px_0_rgb(255_255_255/0.06)]">
      ...
      <div className="flex items-start justify-between">
        ...
        <div className="flex flex-col items-center gap-0.5">
          ...
          <div className="flex flex-col-reverse gap-0.5 rounded border border-[#2a323c] bg-[#0a0e13] p-0.5">
            {RANGE_STEPS.map((r) => (
              <button
                key={r}
                type="button"
                ...
                className={
                  settings.range === r
                    ? 'bevel-in h-4 w-9 font-mono text-[8px] font-semibold'
                    : 'h-4 w-9 rounded-sm bg-[#151b22] font-mono text-[8px] text-knob-green hover:bg-[#1d252e]'
                }
              >
                {r}
              </button>
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col items-center gap-1">
          ...
          <Knob value={settings.gain} min={0} max={110} step={settings.dbStep} onDelta={(d) => adj('gain', d)} color="#7d3038" />
          <span className={val}>{settings.gain.toFixed(1)}dB</span>
        </div>
        <div className="flex flex-col items-center gap-1">
          <span className={lbl}>dB STEP</span>
          <div className="flex overflow-hidden rounded border border-[#2a323c]">
            {GAIN_STEPS.map((s, i) => (
              <button
                key={s}
                type="button"
                ...
                className={
                  'px-1.5 py-0.5 font-mono text-[9px] transition-colors duration-75 ' +
                  (settings.dbStep === s ? 'bg-accent text-white' : 'bg-[#151b22] text-knob-green hover:bg-[#1d252e]') +
                  (i > 0 ? ' border-l border-[#2a323c]' : '')
                }
              >
                {s}
              </button>
      ...
      {/* GATE */}
      <div className="rounded border border-[#2a323c] bg-[#0a0e13] p-1">
        <div className="flex items-center justify-between">
      ...
      <div className="mt-auto flex items-end gap-2">
        <div className="flex h-5 w-6 items-center justify-center rounded-sm border border-[#2a323c] bg-black font-mono text-[7px] text-[#e5484d]">OFF</div>
        <span className="font-mono text-[8px] tracking-widest text-[#5b6673]">USK-7 SIM</span>
  ...
  return (
    <div className="bg-[#161b22] p-2">
      <div className="flex gap-2">
        ...
        <div className="flex w-[84px] flex-col items-center justify-center gap-2">
          <div className="grid grid-cols-3 gap-0.5">
            ...
            <span className="flex h-6 w-6 items-center justify-center rounded-full border border-[#2a323c] bg-[#0d1117] text-[7px] text-white/60">SEL</span>
        ...
        {/* LCD */}
        <div className="flex rounded border border-[#2a323c] bg-black p-1">
          <div>{children}</div>
      ...
      <div className="mt-2 flex items-center justify-center gap-1">
        <span className="mr-2 font-mono text-[9px] tracking-widest text-white/50">EPOCH-SIM 600</span>
        {pkeys.map((p) => (
