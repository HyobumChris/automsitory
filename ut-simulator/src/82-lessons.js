 ...
 *   that advances per start, run through the same mix32(fnv1a(...)) finaliser as the quiz/trade seeds — the plain LCG's
 *   first draw is ~linear in the seed, so consecutive raw seeds would always give the same sign).
 ...
 *   skew 0, focus off, physics + damping points, weldOpts, material carbon, HIDE off, BEAM on, plot points/edge marks —
 *   the beam plot is state a lesson writes (9), so leftovers would pre-satisfy its transition step on a restart); quiz.start() resets the whole
...
(function (UT) {
  ...
  const NEW_LESSONS = {
    ...
    25: { n: 25, title: 'Sensitivity re-check', ko: '감도 재확인: 드리프트 발견, 기준 복원, 재주사 판단', en: 'Sensitivity re-check: spot the drift, restore the reference, decide on re-scanning',
      setup() {
        ...
        L.drift++;
        // §4.1 'sign from the seed': the plain LCG's first draw is ~linear in the seed (0.24…0.26 for 26…60 → always −3),
        // so the counter goes through the same lowbias32 finaliser as the quiz/trade seeds and both signs really occur.
        const sign = M.rng(mix32(M.fnv1a('lesson25:' + L.drift)))() < 0.5 ? -1 : 1;
   ...
   * plus the beam plot (a lesson writes edge marks itself — lesson 9's transition step must not start out already true).
   */
  ...
  api.answer = function (value) {
    const l = ls(), step = curStep(); if (!step) return false;
    // §4.1: only a wrong CHOICE (numeric miss = documented extension) counts as a wrong answer — a plain state step
    // has nothing to answer, so an answer there is a no-op instead of a scored miss.
    if (!step.choices && step.input !== 'number') return false;
    const n = l.active, i = l.step;
